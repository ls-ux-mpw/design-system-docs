...
In cases where you'd use a grid when laying out a screen, use a fixed grid with `16px` gutters – 12-column on desktop with a `max-width` of `960px`, 8 column for tablet with a `max-width` of `720px`, and 4 column for mobile with a `max-width` of `300px`.
